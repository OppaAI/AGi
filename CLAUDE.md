...
1. **SCS (Semantic Cognitive System)** - Provides the core cognitive architecture with memory systems
...
ros2 run scs cnc --ros-args --log-level info
...
- **Input**: `/cns/neural_input` (published by CLI, consumed by CNC)
- **Response**: `/gce/response` (published by CNC, consumed by CLI)
...
5. CNC sends chunks to GUI via `/gce/response`
