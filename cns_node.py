"""
GRACE — CNS Node (Central Nervous System)
==========================================
ROS2 node for Grace's conversational AI.
...
ROS2 Topics:
  Subscribe: /grace/input  (std_msgs/String) — user text
  Publish:   /grace/output (std_msgs/String) — Grace response
"""
...
from simple_sqlite_rag import SimpleSQLiteRAG

import requests

log = logging.getLogger("grace.cns")
...
class CNSNode(Node):
    ...
    def __init__(self):
        ...
        # ── Memory systems ────────────────────────────────────────────────────
        self.rag = SimpleSQLiteRAG(
            db_path=str(Path.home() / GRACE_DB_PATH),
            logger=self.get_logger(),
        )
        self.memory = Memory(
            birth_date=GRACE_BIRTH_DATE,
            rag=self.rag,
        )
        ...
        self._sm_reflections_cache = self.memory.get_recent_reflections_text(
            days=SM_RECENT_DAYS
        )
        ...
        # ── ROS2 pub/sub ──────────────────────────────────────────────────────
        self._pub = self.create_publisher(String, "/grace/output", 10)
        self._sub = self.create_subscription(
            String, "/grace/input", self._on_input, 10
        )
        ...
        self.get_logger().info("✅ CNS Node ready — Grace is awake")
    ...
    def _on_input(self, msg: String):
        ...
        try:
            ...
            user_text = self.memory.scrub(user_text)
            ...
            self.memory.append_user_message(user_text)
            self.memory.append_assistant_message(response)
            ...
            self.get_logger().info(f"🤖 Grace: {response[:80]}")
    ...
    def _build_context(self, user_input: str) -> list[dict]:
        ...
        Priority (lowest to highest — later items dominate):
          1. System prompt + Grace identity
        ...
        # ── 1. System prompt ──────────────────────────────────────────────────
        system = GRACE_SYSTEM.format(birth_date=GRACE_BIRTH_DATE.isoformat())
        ...
        # ── Token trim ───────────────────────────────────────────────────────
        messages = self.memory.trim_to_context_window(messages, MAX_REQUEST_TOKENS)
    ...
    def _retrieve_sm_facts(self, query: str) -> str:
        ...
        keyword_results = self.memory.search_past_memories(query, days_back=None)
        ...
        if has_trigger or not keyword_results:
            # Semantic RAG search
            rag_results = self.rag.search_memory(query, top_k=SM_RAG_TOP_K)
    ...
    def _rebuild_em_summary(self):
        ...
        try:
            today_msgs = self.memory.get_today_messages(date.today().isoformat())
            if not today_msgs:
                return
    ...
    def _consolidate_day(self, date_str: str):
        ...
        with self._consolidation_lock:
            try:
                ...
                self.rag.archive_day_to_rag(date_str, daily_reflection=reflection)
    ...
    def _recover_em_on_startup(self):
        ...
        try:
            ...
            if not self.memory.reflection_exists(yesterday):
                msgs = self.memory.get_today_messages(yesterday)
                if msgs:
    ...
    def graceful_shutdown(self):
        ...
        self.rag.close()
        self.get_logger().info("👋 Grace going to sleep. Goodbye.")
