...
### Week 1: Project Genesis & Foundation (Mar 16 – Mar 22)
- Mar 16-17: Initialized the clean-slate ROS workspace and the scs package. Officially commenced Milestone 1: Chatbot with Memory.
- Mar 18-22: Established the Homeostatic Regulation Parameters (HRP) as the single source of truth for tunable cognitive settings. Adopted a humanistic/neuroscience naming convention across the codebase (e.g., WMC, PMT).
### Week 2: Memory Wiring & Coordination (Mar 23 – Mar 29)
- Mar 23-27: Built the Memory Coordination Cortex (MCC) to manage the relay of receding PMT schemas from WMC to EMC. Assembled the memory context builder.
- Mar 28-29: Abstracted EMC parameters into HRP and transitioned the underlying encoding engine from lazy to eager load.
### Week 3: Search Mechanics & Buffer Design (Mar 30 – Apr 05)
- Mar 30-Apr 02: Refactored the Encoding Engine and EMC initialization. Introduced dual-path processing by calculating both semantic match (cosine similarity) and lexical metrics.
- Apr 03-05: Redesigned the episodic buffer in EMC to share functions between intake and recall streams. Replaced native Python similarity calculations with sqlite-vec L2 distance semantic search.
### Week 4: Deep Storage & Context Pairing (Apr 06 – Apr 12)
- Apr 06-08: Fully integrated sqlite-vec into the memory pipeline. Refactored WMC to EMC binding buffers. Optimized thread safety and CPU priorities for non-latency-sensitive encoder threads. Added timing and token tracking.
- Apr 09-12: Addressed context fragmentation by redesigning PMT handling. Ensured user prompts and AI responses are paired together as single complete interactions within the working memory and engram schemas.
### Week 5: Architecture Abstraction & Engram Complex (Apr 13 – Apr 19)
- Apr 13-15: Refined the 3-table SQLite design and implemented FTS5 for enhanced lexical recall. Adopted crash-safe JSON serialization for PMT content.
- Apr 16-19: Created the Memory Storage Bank (MSB) to centralize engram access logic for future cortices (SMC/PMC). Abstracted encoding schemas, introducing generic classes to decouple SQL generation from biological memory logic.
### Week 6: Refinement & Advanced Retrieval (Apr 20 – Apr 26)
- Apr 20-22: Upgraded embedding models to BGE-base-en-v1.5 and introduced asymmetric cue prefixing to dramatically improve retrieval accuracy. Streamlined MSB logic.
- Apr 23-26: Renamed MSB to Engram Complex (ECX) to align with neuroscience terminology. Decoupled engram and cue encoding, employing deterministic MD5 hashing for primary keys. Fused semantic and lexical results in a consolidated recall_engram method.
### Week 7: GCE Integration & Polish (Apr 27 – May 03)
- Apr 27-29: Finalized episodic memory staging and indexing. Migrated the encoding cycle mechanism to simulate "theta rhythm" processing. Injected configuration parameters globally through HRP.
Apr 30-May 03: Wired up the Cognitive Navigation Cortex (CNC) with the Generative Cognitive Engine (GCE). Implemented VRAM keep-alive features, chunk capacity - limits, and context truncation to prevent OOM errors. Upgraded the underlying LLM to HammerAI/mn-mag-mell-r1 and stabilized system deployments.
